...
async function main() {
  ...
  for (const user of users) {
    try {
      ...
      const reqHeaders = { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/x-www-form-urlencoded' };
      const createRes = await fetch('http://localhost:5000/api/surat', { method: 'POST', headers: reqHeaders, body: fd.toString() });
