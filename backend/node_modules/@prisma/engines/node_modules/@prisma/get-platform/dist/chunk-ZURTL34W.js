...
var chunk_ZURTL34W_exports = {};
__export(chunk_ZURTL34W_exports, {
...
module.exports = __toCommonJS(chunk_ZURTL34W_exports);
...
function parseDistro(osReleaseInput) {
  ...
  const distroInfo = M({ id, idLike }).with(
    { id: "alpine" },
    ({ id: originalDistro }) => ({
      targetDistro: "musl",
...
function computeLibSSLSpecificPaths(args) {
  return M(args).with({ familyDistro: "musl" }, () => {
...
function getBinaryTargetForCurrentPlatformInternal(args) {
  ...
  if (platform === "linux" && libssl === void 0) {
    const additionalMessage = M({ familyDistro }).with({ familyDistro: "debian" }, () => {
